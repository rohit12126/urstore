...
<form role="form" class="form-horizontal tasi-form" action="<?php echo current_url()?>" enctype="multipart/form-data" method="post" id="form_valid">
  ...
  $comment=$comment[0];
    ...
    <div class="form-group">
      ...
      <div class="col-md-6"><?php echo date('d M Y,h:i  A',strtotime($comment->updated)); ?></div>
    </div>
    <div class="form-group">
         ...
         <div class="col-md-6">
           <select name="user_id" class="form-control"  id="user_id">
                  ...
                  <?php foreach ($users as $user): ?>
                     <option value="<?php echo $user['ID'] ?>" <?php if(!empty($_POST['user_id']) && $user['ID']==$_POST['user_id']) { echo "selected"; } else if($comment->user_id==$user['ID']) echo "selected";?>>
    ...
    <div class="form-group">
         ...
         <div class="col-md-6">
           <select name="blog_id" class="form-control"  id="blog_id">
                  ...
                  <?php foreach ($blogs as $blog): ?>
                     <option value="<?php echo $blog['id'] ?>" <?php if(!empty($_POST['blog_id']) && $blog['id']==$_POST['blog_id']) echo "selected"; else if($comment->blog_id==$blog['id']) echo "selected"; ?>>
     ...
     <div class="form-group">
      ...
      <div class="col-md-6">
        <textarea name="comment" rows="3" class="form-control" placeholder="Blog Comment" data-bvalidator="required" data-bvalidator-msg="Blog Comment required"><?php if ($this->input->post('comment')){ echo set_value('comment'); } else { echo $comment->comment;} ?></textarea>
  ...
  <div class="form-actions fluid">
    <div class="col-md-offset-2 col-md-10">
      ...
      <button  class="btn btn-primary tooltips" rel="tooltip" data-placement="top" data-original-title="Add New Comment" type="submit"> <i class="icon-plus"></i> Edit Comment</button>
      </div>
    </div>
  </form>
</div>                     
