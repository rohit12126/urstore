...
class Blog_category extends CI_Controller {
  ...
  {	
	 ...
	 if ($this->input->post('action_type')==1) 
    {
      ...
      $data['update']=$this->product_category_model->getColumnDataWhere('product_category','',array('id'=>$id),'','');
      ...
      $this->form_validation->set_rules('category_name', 'category name', 'trim|required|callback_check_updateproductcategory');	
      ...
      if($this->form_validation->run()==TRUE)
      {
        ...
        if($this->common_model->update('product_category',$inset,array('id'=>$id)))
        {
          ...
          $this->session->set_flashdata('msg_success', 'Post category updated successfully.');
        }
        else
        {
          $this->session->set_flashdata('msg_error','Post Category update failed, Please try again.');
        }
    ...
    $this->form_validation->set_rules('description', 'Description', 'trim|required|max_length[200]'); 
      ...
      $inset['description']=$this->input->post('description');
			...
			{
			$this->session->set_flashdata('msg_error', 'New add Blogcategory failed, Please try again.');
			}
			redirect('post_category');
		}
   }
  ...
  public function check_updateproductcategory($str)
  {
    ...
    $check=$this->product_category_model->getColumnDataWhere('product_category','',array('id !='=>$id,'category_name'=>$str),'','');
    if(count($check)>0)
    { 
  ...
  {
    if(empty($id)) redirect('post_category/');
    if($status==0){
        $cat_status=1;
    }
    ...
    redirect('post_category/index/'.$offset);  
  }
