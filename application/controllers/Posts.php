...
/* 		 if($this->common_model->admin_logged_in()===FALSE)
  ...
  public function index($offset=0)  
  {
    ...
    if(isset($_POST) && !empty($_POST['order_number']) && !empty($_POST['id']))
    {
    	for($i=0;$i<sizeof($_POST['order_number']);$i++)
    	{
    		$this->common_model->update('blogs',array('order_number'=>$_POST['order_number'][$i]),array('id'=>$_POST['id'][$i]));
    	}
    	$this->session->set_flashdata('msg_success','All Post order number updated successfully.');
		redirect('posts');
    }
  ...
  /*  if ($this->input->get('search')) 
   ...
   public function post_add()
    {    
		...
		$this->form_validation->set_rules('short_description','Post Short Description','required|max_length[200]');
		...
		if ($this->form_validation->run() == TRUE){
			...
			$data_insert['short_description']=$this->input->post('short_description');
    ...
    {    
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			$data_insert['blog_slug']	=	url_title($this->input->post('blog_title'), '-', TRUE);
			$data_insert['updated'] =	date('Y-m-d h:i:s A');
	        if($this->superadmin_model->update('blogs',$data_insert,$array = array('id' => $id, )))
	        {
	...
	public function post_tags() 
	{
        ...
        $data['template']='backend/post/post_tags';
        $this->load->view('templates/backend_template',$data);
    }

   public function post_tag_delete($id='')
    {
    	$this->_check_login(); //check login authentication
        if(empty($id)){ redirect('posts/post_tags'); }
    ...
    public function comments($offset=0)
  	{
	    ...
	    $config=backend_pagination();
	    $config['base_url'] = base_url().'posts/comments';
	    ...
	    $data['template']='backend/post/comments';
	    $this->load->view('templates/backend_template',$data);
    }
    public function comment_delete($id='')
    {
    	$this->_check_login(); //check login authentication
        if(empty($id)){ redirect('backend/post/comments/'); }
    ...
    {    
    	...
    	$data['template']='backend/post/comment_add';
   		$this->load->view('templates/backend_template',$data);
    }	
    ...
    {    
		...
		$data['blogs'] = $this->superadmin_model->getAlldata('blogs');
		$data['users'] = $this->superadmin_model->getAlldata('l7d1zw_users');
    	$data['template']='backend/post/comment_edit';
   		$this->load->view('templates/backend_template',$data);
    }
	function abc()
	{
		$result=$this->common_model->getAlldata('product_category','','',array('status',1));
